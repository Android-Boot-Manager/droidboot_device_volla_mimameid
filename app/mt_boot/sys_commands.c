...
void cmd_oem_ap_jtag(const char *arg, void *data, unsigned sz)
{
	...
	if (!strncmp(arg, " 1", strlen(" 1"))) {
		...
		int n = snprintf(response, MAX_RSP_SIZE, "\tCurrent AP JTAG setting:%s",
...
void cmd_oem_scp_status(const char *arg, void *data, unsigned sz)
{
	...
	if (!strncmp(arg, " 1", strlen(" 1"))) {
		...
		int n = snprintf(response, MAX_RSP_SIZE, "\tCurrent SCP setting:%s",
...
void cmd_oem_scp_log_thru_ap_uart(const char *arg, void *data, unsigned sz)
{
	...
	if (!strncmp(arg, " 1", strlen(" 1"))) {
		...
		int n = snprintf(response, MAX_RSP_SIZE, "Get SCP log thru AP UART: %s",
...
void cmd_oem_usb2jtag(const char *arg, void *data, unsigned sz)
{
	...
	if (!strncmp(arg, " 1", strlen(" 1"))) {
		...
		int n = snprintf(response, MAX_RSP_SIZE, "\tCurrent USB2JTAG setting:%s",
...
void cmd_getvar(const char *arg, void *data, unsigned sz)
{
	...
	if (!strcmp(arg, "all")) {
		for (var = varlist; var; var = var->next) {
			int n = snprintf(response, MAX_RSP_SIZE, "%s: %s", var->name, var->value);
			if (n < 0) {
				strncpy(response, "unknown error", sizeof(response) - 1);
				response[sizeof(response) - 1] = '\0';
			}
...
#define CMDLINE_TMP_CONCAT_SIZE 100     //only for string concat, 100 bytes is enough
	...
	n = snprintf(cmdline_tmpbuf, CMDLINE_TMP_CONCAT_SIZE, "lcm=%1d-%s",
		 DISP_IsLcmFound(), mt_disp_get_lcm_id());
	if (n < 0)
		dprintf(INFO, "cmdline snprintf error\n");
	else
		cmdline_append(cmdline_tmpbuf);

	n = snprintf(cmdline_tmpbuf, CMDLINE_TMP_CONCAT_SIZE, "fps=%1d",
	...
	n = snprintf(cmdline_tmpbuf, CMDLINE_TMP_CONCAT_SIZE, "vram=%1d",
		 DISP_GetVRamSize());
	if (n < 0)
		dprintf(INFO, "cmdline snprintf error\n");
	else
