...
{
	if (sprintf(battery_voltage, "%dmV", get_bat_volt(1)) < 0)
		dprintf(INFO, "sprintf error in %s:%d\n", __FILE__, __LINE__);

	if (sprintf(hw_revision, "%x", mt_get_chip_hw_ver()) < 0)
		dprintf(INFO, "sprintf error in %s:%d\n", __FILE__, __LINE__);

#ifndef MNTL_SUPPORT
	if (sprintf(erase_blk_size, "0x%lx", mt_part_get_erasesz()) < 0)
		dprintf(INFO, "sprintf error in %s:%d\n", __FILE__, __LINE__);

	if (sprintf(logical_blk_size, "0x%lx", mt_part_get_blksz()) < 0)
		dprintf(INFO, "sprintf error in %s:%d\n", __FILE__, __LINE__);
#endif
}
...
#endif
	if (len <= 0) {
		if (sprintf(radio_version, "N/A") < 0)
			dprintf(INFO, "sprintf error in %s:%d\n", __FILE__, __LINE__);
	}
}
...
/* Remove all published variables which name with its length is matched*/
void fastboot_delete_var(const char *name)
{
	struct fastboot_var *var = varlist;
	struct fastboot_var *var_prev = NULL, *var_remove;
...
void fastboot_ack(const char *code, const char *reason)
{
	...
	if (snprintf(response, MAX_RSP_SIZE, "%s%s", code, reason) < 0) {
...
void fastboot_info(const char *reason)
{
	...
	if (snprintf(response, MAX_RSP_SIZE, "INFO%s", reason) < 0) {
		strncpy(response, "unknown error", sizeof(response) - 1);
		response[sizeof(response) - 1] = '\0';
	}
...
void register_partition_var(void)
{
	...
	if (is_registered) {
		fastboot_delete_var(part_type);
		fastboot_delete_var(part_size);
	}
	...
	for (i=0; i<PART_MAX_COUNT; i++) {
		...
		if (snprintf(var_name_buf, buf_len, "%s%s", part_type, p_name_buf) < 0) {
		...
		if (snprintf(value_buf,val_buf_len,"%llx",p_size) < 0) {
			dprintf(INFO, "snprintf error in %s:%d\n", __FILE__, __LINE__);
			continue;
		}
		...
		if (snprintf(var_name_buf, buf_len, "%s%s", part_size, p_name_buf) < 0) {
			dprintf(INFO, "snprintf error in %s:%d\n", __FILE__, __LINE__);
			continue;
		}
...
static unsigned int part_name_index = 0;
...
{
	for (unsigned int i = 0; i < PART_MAX_COUNT; i++) {
...
{
	for (unsigned int i = 0; i < part_name_index; i++) {
...
static void publish_ab_variables(void)
{
	...
	for (int i = 0; i < PART_MAX_COUNT; i++) {
		...
		if (snprintf(new_name, len, "%s%s", str_has_slot, part_name) < 0) {
			dprintf(INFO, "snprintf error in %s:%d\n", __FILE__, __LINE__);
			continue;
		}
		if (new_name[len - 3] == '_') {
	...
	if (ab_suffix) {
		...
		dprintf(INFO, "ab_suffix is null!\n");
	...
	if (snprintf(str_retry_count_a, sizeof(str_retry_count_a), "%d", (int)retry_count) < 0)
		dprintf(INFO, "snprintf error in %s:%d\n", __FILE__, __LINE__);
	else
		fastboot_publish(str_retry_a, str_retry_count_a);

	retry_count = get_retry_count("_b");
	if (snprintf(str_retry_count_b, sizeof(str_retry_count_b), "%d", (int)retry_count) < 0)
		dprintf(INFO, "snprintf error in %s:%d\n", __FILE__, __LINE__);
	else
...
int fastboot_init(void *base, unsigned size)
{
	...
	if (snprintf(dl_max_str, sizeof(dl_max_str), "0x%X", download_max) < 0)
		dprintf(INFO, "snprintf error in %s:%d\n", __FILE__, __LINE__);
