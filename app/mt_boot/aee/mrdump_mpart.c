...
static int Do_Store_or_Write(struct mrdump_part_handle *handle, uint8_t *buf, uint32_t Length)
{
	...
	memcpy(handle->data + handle->midx, buf, mylen);
	handle->midx += mylen;
...
int mrdump_partition_output(const struct mrdump_control_block *mrdump_cb, const struct kzip_addlist *memlist, const struct kzip_addlist *memlist_cmm, struct mrdump_dev *mrdump_dev)
{
...
int mrdump_gpt_resize(int method)
{
	...
	if (err || is_gpt_valid(data, 1) != 0) {
		err = read_data(data, part_id, last_lba(part_id), sz);
		if (err || is_gpt_valid(data, last_lba(part_id)) != 0) {
	...
	mrdump_index = get_gpt_entry_index("mrdump");
	if (mrdump_index < 0) {
	...
	data_index = get_gpt_entry_index("userdata");
	if (data_index < 0) {
	...
	if ((method == 1) && (partition_erase("userdata") < 0)) {
		voprintf_error("userdata partition erase failed\n");
	}
...
{
	if (get_gpt_entry_index("mrdump") != -1) {
