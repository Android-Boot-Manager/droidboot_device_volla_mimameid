...
static uint8_t *mrdump_core32_write_cpu_note(const struct mrdump_control_block *mrdump_cb,
					     unsigned int cpu, struct elf32_phdr *nhdr,
					     uint8_t *bufp)
{
	if (cpu >= MRDUMP_CPU_MAX) {
		/* cpu value is not correct, ignore it */
		return bufp;
	}
	...
	ASSERT(snprintf(cpustr, sizeof(cpustr), "CPU%d", cpu) > 0);
	...
	memcpy(&prstatus.pr_reg, (unsigned long*)&mrdump_cb->crash_record.cpu_regs[cpu].arm32_regs,
...
static uint8_t *mrdump_core64_write_cpu_note(const struct mrdump_control_block *mrdump_cb,
					     unsigned int cpu, struct elf64_phdr *nhdr, uint8_t *bufp)
{
	if (cpu >= MRDUMP_CPU_MAX) {
		/* cpu value is not correct, ignore it */
		return bufp;
...
static uint8_t *mrdump_core64_write_vmcoreinfo(const struct mrdump_control_block *mrdump_cb, struct elf64_phdr *nhdr, uint8_t *bufp)
{
	...
	ASSERT(snprintf(vmcoreinfo, sizeof(vmcoreinfo),
			"PAGESIZE=4096\n"
			...
			39,
			(uint64_t)kparams->phys_offset) > 0);
