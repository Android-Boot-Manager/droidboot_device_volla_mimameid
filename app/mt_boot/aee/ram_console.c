...
static int cur_boot = BOOT_INVALID;
...
bool ram_console_get_kaslr_offset(uint64_t* kaslr_offset)
{
	if (krbr && U_VAR(set_flag)) {
		if (krbr->kaslr_offset == 0xecab1e) {
			*kaslr_offset = 0;
			return false;
		}

		if (krbr->kaslr_offset == 0xd15ab1e) {
			*kaslr_offset = 0;
		} else {
...
/*
 * The return value of ram_console_is_abnoraml_boot() maybe changed if
 * ram_console was restored by kedump_restore_mem(). So we recored the value
 * in cur_boot before ram_console restored.
 * The current_is_abnormal_boot() would return the value of cur_boot.
 */
int current_is_abnormal_boot(void)
{
	if (cur_boot == BOOT_NORMAL || cur_boot == BOOT_ABNORMAL)
		return cur_boot;

	cur_boot = ram_console_is_abnormal_boot() ? BOOT_ABNORMAL : BOOT_NORMAL;
