...
#endif
#ifdef MTK_PICACHU_SUPPORT
#ifdef MTK_PICACHU_SUPPORT_DUMP_SYS_PI
        [AEE_PLAT_PICACHU_LOG] = { "SYS_PICACHU_LOG", 0x17F000, AEE_LKDUMP_PICACHU_LOG}, /*  1020KB + 512 KB */
#else
	[AEE_PLAT_PICACHU_LOG] = { "SYS_PICACHU_LOG", 0x80000, AEE_LKDUMP_PICACHU_LOG}, /*  512 KB */
#endif
#endif
#ifdef MICROTRUST_TEE_DUMP_SUPPORT
	[AEE_PLAT_TEEI_LOG] = { "SYS_TEEI_LOG", 0x20000, AEE_LKDUMP_TEEI_LOG}, /* 128 KB */
#endif
	[AEE_PLAT_ATF_FOOTPRINT_LOG] = { "SYS_ATF_FOOTPRINT", 0x800, AEE_LKDUMP_ATF_FOOTPRINT}, /* 2KB */
#ifdef MTK_DPM_SUPPORT
	[AEE_PLAT_DPM_DATA] = { "SYS_DPM_DATA", 0x10400, AEE_LKDUMP_DPM_DATA}, /* 65KB */
#endif
#ifdef MTK_GPU_DFD_SUPPORT
	[AEE_PLAT_GPU_DFD] = { "GPU.dfd", 0x100000, AEE_LKDUMP_GPU_DFD}, /* 1024KB */
#endif
};
...
extern uint64_t v2p_32(uint64_t vptr);
...
uint64_t kedump_mem_read(uint64_t data, unsigned long sz, void *buf)
{
	...
	if(!is_dram_address(data, &g_mcb) || !is_dram_address((data + sz - 1), &g_mcb)) {
...
static unsigned long long kedump_dev_write_va_32(
		unsigned long long offset, uint64_t vaddr, unsigned long size)
{
	unsigned long long paddr;

	paddr = v2p_32(vaddr);
	if (paddr && is_dram_address(paddr, &g_mcb))
		return kedump_dev_write(offset, (uint64_t)paddr, size);
...
static unsigned int kedump_mini_rdump(struct elfhdr *ehdr, unsigned long long offset)
{
		...
		if (size != 0 && elfoff != 0) {
			...
			if (!is_arm_32bit(&g_mcb, vaddr)
				...
				if (addr)
					...
					total += kedump_dev_write_va_32(offset + elfoff, vaddr, size);
			} else {
				...
				if (!addr)
					addr = v2p_64(vaddr);
				if (!addr)
					LOG("kedump: convert fail va 0x%llx, size 0x%lx\n", vaddr, size);
				total += kedump_dev_write(offset + elfoff, (uint64_t)addr, size);
			}
		}
		elfoff += size;
	...
	LOG("kedump: writed size 0x%x\n", total);
	return (unsigned int)elfoff;
}
	...
	if (iheader.magic == AEE_IPANIC_MAGIC && iheader.version >= AEE_IPANIC_PHDR_VERSION) {
		...
		for (i = IPANIC_NR_SECTIONS - 1; i >= 0; i--) {
			...
			if (strncmp((const char *)iheader.data_hdr[i].name, "SYS_RAMCONSOLE_RAW", sizeof("SYS_RAMCONSOLE_RAW") - 1) == 0) {
				...
				if (addr_misc_pstore == 0 || sz_misc_pstore == 0)
					continue;
