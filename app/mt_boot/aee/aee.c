...
int mrdump_get_env(const char *name, char *out, int out_len)
{
	...
	struct mrdump_dev *blkpart = mrdump_dev_blkpart(AEE_IPANIC_LABEL);

	if (blkpart == NULL) {
		dprintf(CRITICAL, "%s partition does not exist\n", AEE_IPANIC_LABEL);
		return -ENODEV;
	}

	off_t part_total_size = partition_get_size_by_name(AEE_IPANIC_LABEL);
	off_t offset;
	bool  status;

	offset = part_total_size - MRDUMP_CONFIG_EXPDB_OFFSET;
	status = blkpart->read(blkpart, offset, (uint8_t *)&mrdump_config, sizeof(struct mrdump_expdb_config));
	free(blkpart);
	if (!status) {
		dprintf(CRITICAL, "%s: bio_read fail\n", __func__);
		return -EIO;
	}

	if (!strncmp(name, MRDUMP_OUTPUT, sizeof(MRDUMP_OUTPUT))) {
		if (strlcpy(out, mrdump_config.output_dev, out_len) >= out_len) {
			dprintf(CRITICAL, "%s: mrdump_config.output_dev too long\n", __func__);
			return -1;
		}
		dprintf(CRITICAL, "%s: %s\n", MRDUMP_OUTPUT, out);
	} else if (!strncmp(name, MRDUMP_ALLOCATE_SIZE, 20)) {
		if (strlcpy(out, mrdump_config.allocate_size, out_len) >= out_len) {
			dprintf(CRITICAL, "%s: mrdump_config.allocate_size too long\n", __func__);
			return -1;
		}
		dprintf(CRITICAL, "%s: %s\n", MRDUMP_ALLOCATE_SIZE, out);
	} else if (!strncmp(name, MRDUMP_MEM_SIZE, sizeof(MRDUMP_MEM_SIZE))) {
		if (strlcpy(out, mrdump_config.mem_size, out_len) >= out_len) {
			dprintf(CRITICAL, "%s: mrdump_config.mem_size too long\n", __func__);
			return -1;
		}
	} else if (!strncmp(name, MRDUMP_LBAOOO, sizeof(MRDUMP_LBAOOO))) {
		if (strlcpy(out, mrdump_config.lbaooo, out_len) >= out_len) {
			dprintf(CRITICAL, "%s: mrdump_config.lbaooo too long\n", __func__);
			return -1;
		}
	} else if (!strncmp(name, MRDUMP_DUMP_TYPE, sizeof(MRDUMP_DUMP_TYPE))) {
		if (strlcpy(out, mrdump_config.dump_type, out_len) >= out_len) {
			dprintf(CRITICAL, "%s: mrdump_config.dump_type too long\n", __func__);
			return -1;
		}
	} else {
		dprintf(CRITICAL, "unknown mrdump config %s\n", name);
		return -1;
	}
	dprintf(CRITICAL, "%s: %s\n", name, out);
	return 0;
...
int mrdump_set_env(const char *name, const char *value)
{
	...
	memset(&mrdump_config, 0, sizeof(mrdump_config));
	struct mrdump_dev *blkpart = mrdump_dev_blkpart(AEE_IPANIC_LABEL);
	...
	part_total_size = partition_get_size_by_name(AEE_IPANIC_LABEL);
	offset = part_total_size - MRDUMP_CONFIG_EXPDB_OFFSET;
	status =  blkpart->read(blkpart, offset, (uint8_t *)&mrdump_config, sizeof(struct mrdump_expdb_config));
	if (!status) {
		dprintf(CRITICAL, "%s: bio_read fail\n", __func__);
	...
	if (!strncmp(name, MRDUMP_OUTPUT, sizeof(MRDUMP_OUTPUT))) {
		memcpy(&mrdump_config.output_dev, value, sizeof(mrdump_config.output_dev));
	} else if (!strncmp(name, MRDUMP_ALLOCATE_SIZE, sizeof(MRDUMP_ALLOCATE_SIZE))) {
		memcpy(&mrdump_config.allocate_size, value, sizeof(mrdump_config.allocate_size));
	} else if (!strncmp(name, MRDUMP_MEM_SIZE, sizeof(MRDUMP_MEM_SIZE))) {
		memcpy(&mrdump_config.mem_size, value, sizeof(mrdump_config.mem_size));
	} else if (!strncmp(name, MRDUMP_LBAOOO, sizeof(MRDUMP_LBAOOO))) {
		memcpy(&mrdump_config.lbaooo, value, sizeof(mrdump_config.lbaooo));
	} else if (!strncmp(name, MRDUMP_DUMP_TYPE, sizeof(MRDUMP_DUMP_TYPE))) {
		memcpy(&mrdump_config.dump_type, value, sizeof(mrdump_config.dump_type));
	} else {
		dprintf(CRITICAL, "unknown mrdump config\n");
		free(blkpart);
		return -1;
	}
	dprintf(CRITICAL, "%s update to %s\n", name, value);

	status =  blkpart->write(blkpart, offset, (uint8_t *)&mrdump_config, sizeof(struct mrdump_expdb_config));
	if (!status) {
		dprintf(CRITICAL, "%s: bio_write fail\n", __func__);
		free(blkpart);
		return -EIO;
	}
	free(blkpart);
...
void voprintf(char type, const char *msg, va_list ap)
{
	...
	*p++ = type;
	*p++ = ':';
	if (vsnprintf(p, sizeof(msgbuf) - (p - msgbuf), msg, ap) < 0) {
		strlcpy(p, "<Message format error>\n", p  - msgbuf);
	}
...
static uint64_t mrdump_mem_size(void)
{
	...
	if (mrdump_get_env("mrdump_mem_size", mem_size_param, sizeof(mem_size_param)))
		dprintf(CRITICAL, "%s: mrdump_get_env fail\n", __func__);
...
int mrdump_set_output_device(const char *output_dev)
{
	...
	if (retval != MRDUMP_DEV_UNKNOWN) {
		if (mrdump_set_env("mrdump_output", (char *)output_dev) == 0) {
...
{
	video_clear_image_by_color(0xFFFF7F00, 0, 0, CFG_DISPLAY_WIDTH, CFG_DISPLAY_HEIGHT);
	video_set_bgcolor(0xFFFF7F00);
	video_set_fgcolor(0xFFFF0000);
	...
	switch (output_device) {
		...
		case MRDUMP_DEV_NULL:
			dump_ok = mrdump_null_output(mrdump_cblock, memlist, memlist_cmm);
			break;
		case MRDUMP_DEV_ISTORAGE:
			dump_ok = mrdump_ext4_output(mrdump_cblock, memlist, memlist_cmm, mrdump_dev_blkpart("userdata"));
			break;
		case MRDUMP_DEV_USB:
			dump_ok = mrdump_usb_output(mrdump_cblock, memlist, memlist_cmm);
			break;
		case MRDUMP_DEV_PARTITION:
			dump_ok = mrdump_partition_output(mrdump_cblock, memlist, memlist_cmm, mrdump_dev_blkpart("mrdump"));
			break;
		default:
...
int mrdump_detection(void)
{
	...
	if (mrdump_cb->enabled != MRDUMP_ENABLE_COOKIE) {
	...
	switch (reboot_mode) {
		...
		case AEE_REBOOT_MODE_NORMAL: {
			...
			if (ram_console_reboot_by_mrdump_key && ram_console_reboot_by_mrdump_key()) {
				mrdump_cb->crash_record.reboot_mode = AEE_REBOOT_MODE_MRDUMP_KEY;
...
error:
	...
	if (output_device == MRDUMP_DEV_USB) {
		...
		if (mrdump_set_env("mrdump_output", "usb")) {
