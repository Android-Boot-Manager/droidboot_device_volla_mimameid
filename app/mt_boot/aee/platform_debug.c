...
unsigned int (* plat_atf_footprint_log_get)(u64 offset, int *len, CALLBACK dev_write) = NULL;
...
#endif
#ifdef MICROTRUST_TEE_DUMP_SUPPORT
unsigned int (* plat_teei_log_get)(u64 offset, int *len, CALLBACK dev_write) = NULL;
#endif
#ifdef MTK_DPM_SUPPORT
unsigned int (* plat_dpm_data_get)(u64 offset, int *len, CALLBACK dev_write) = NULL;
#endif
#ifdef MTK_GPU_DFD_SUPPORT
unsigned int (* plat_gpu_dfd_get)(u64 offset, int *len, CALLBACK dev_write) = NULL;
#endif
...
unsigned int kedump_plat_savelog(int condition, u64 offset, int *len, CALLBACK dev_write)
{
	switch (condition) {
		...
		case AEE_PLAT_ATF_FOOTPRINT_LOG:
			return (!plat_atf_footprint_log_get ? 0 : plat_atf_footprint_log_get(offset, len, dev_write));
		case AEE_PLAT_HVFS:
...
#endif
#ifdef MICROTRUST_TEE_DUMP_SUPPORT
		case AEE_PLAT_TEEI_LOG:
			return (!plat_teei_log_get ? 0 : plat_teei_log_get(offset, len, dev_write));
#endif
#ifdef MTK_TINYSYS_MCUPM_SUPPORT
		case AEE_PLAT_MCUPM_COREDUMP:
			return (!plat_mcupm_coredump_get ? 0 : plat_mcupm_coredump_get(offset, len, dev_write));
		case AEE_PLAT_MCUPM_DATA:
			return (!plat_mcupm_data_get ? 0 : plat_mcupm_data_get(offset, len, dev_write));
		case AEE_PLAT_MCUPM_XFILE:
			return (!plat_mcupm_xfile_get ? 0 : plat_mcupm_xfile_get(offset, len, dev_write));
		case AEE_PLAT_MCUPM_LAST_LOG:
			return (!plat_mcupm_log_get ? 0 : plat_mcupm_log_get(offset, len, dev_write));
#endif
#ifdef MTK_DPM_SUPPORT
		case AEE_PLAT_DPM_DATA:
			return (!plat_dpm_data_get ? 0 : plat_dpm_data_get(offset, len, dev_write));
#endif
#ifdef MTK_GPU_DFD_SUPPORT
		case AEE_PLAT_GPU_DFD:
			return (!plat_gpu_dfd_get ? 0 : plat_gpu_dfd_get(offset, len, dev_write));
#endif
