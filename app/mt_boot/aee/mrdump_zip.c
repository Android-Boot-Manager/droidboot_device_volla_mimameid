...
static char zero_page[PAGE_SIZE] __attribute__((aligned(PAGE_SIZE))) = {0};
		    ...
		    uint64_t paddr, vaddr_t vaddr, unsigned long sz, uint64_t zero_pa)
{
	...
	page_start = vaddr + sz + offset; /* page struct map to vaddr + sz!!! */
	for (i = -offset; i < memmap_sz; i += PAGE_SIZE) {
		pa = v2p_64(memmap + i);
		if (pa < DRAM_PHY_ADDR
		  || arch_mmu_map(pa, page_start + i, mapflags, PAGE_SIZE) != NO_ERROR) {
			paddr = pa;
			vaddr = page_start - offset + i;
			goto fail;
		}
	}
	/* map DDR according memmap flag */
	for (i = 0; i < memmap_sz; i += struct_page_size, vaddr += PAGE_SIZE, paddr += PAGE_SIZE) {
		if (*(uint64_t *)(page_start + i)&pageflags) {
			if (arch_mmu_map(zero_pa, vaddr, mapflags, PAGE_SIZE) == NO_ERROR) {
				invalid_cnt++;
				continue;
			}
		}

		if (arch_mmu_map(paddr, vaddr, mapflags, PAGE_SIZE) != NO_ERROR)
			goto fail;
	}
	return invalid_cnt;
fail:
	voprintf_debug("map error: paddr=0x%016llx, vaddr=0x%08x, zero_pa=0x%016llx\n", paddr, vaddr, zero_pa);
	return -1;
}
...
static bool is_linux_kernel_memory_dump(const struct kzip_addlist *addlist)
{
	...
	if (strcmp(dump_type, MRDUMP_LINUX_KERNEL_MEMORY) == 0) {
		if ((memmap > 0) && (pageflags > 0) && (struct_page_size > 0)) {
			voprintf_debug("dump_linux_kernel_memory = 1, memmap(0x%llx), pageflags(0x%llx), struct_page_size(0x%x)\n",
					memmap, pageflags, struct_page_size);
			return true;
		}
	}
	voprintf_debug("dump_linux_kernel_memory = 0, memmap(0x%llx), pageflags(0x%llx), struct_page_size(0x%x)\n",
			memmap, pageflags, struct_page_size);
	return false;
}
                                    ...
                                    struct kzip_entry *zentry, z_stream *strm, struct aee_timer *zip_time)
{
	...
	voprintf_debug("-- map: paddr=0x%016llx, vaddr=0x%08x, zero_page=%p\n", paddr, vaddr, zero_page);
	...
	*flush = (memlist->size == 0) ? Z_FINISH : Z_NO_FLUSH;
	...
	bool dump_linux_kernel_memory = is_linux_kernel_memory_dump(memlist);

	do {
		/* arch_mmu_map by each 2MB size */
		if (cnt == 0) {
			if (dump_linux_kernel_memory)
				invalid_cnt = mrdump_mem_map(memlist, paddr, vaddr, SECTION_SIZE, v2p_64((uint64_t)(uintptr_t)zero_page));
	...
	voprintf_debug("-- Compress memory %llx, done. surplus size: %lld, ignore: %dMB\n", memlist->address, mem_size, tot_invalid_cnt * 4 / 1024);
	return true;
}
#endif
