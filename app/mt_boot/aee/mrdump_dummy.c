...
{
#ifdef PL_DRAMBUF_BASE
	if (mblock_query_reserved(&g_boot_arg->mblock_info,
				  "pl-drambuf", 0)) {
		if (mblock_create(&g_boot_arg->mblock_info,
				  &g_boot_arg->orig_dram_info,
				  PL_DRAMBUF_BASE, PL_DRAMBUF_MAX_SIZE)) {
			dprintf(CRITICAL, "%s: free PL_DRAMBUF_ARG failed\n", __func__);
			assert(0);
		}
	}
#endif /* end of PL_DRAMBUF_BASE */
#ifdef PL_BOOTTAGS_BASE
	if (mblock_query_reserved(&g_boot_arg->mblock_info,
				  "pl-boottag", 0)) {
		if (mblock_create(&g_boot_arg->mblock_info,
				  &g_boot_arg->orig_dram_info,
				  PL_BOOTTAGS_BASE, PL_BOOTTAGS_MAX_SIZE)) {
			dprintf(CRITICAL, "%s: free PL_BOOT_ARG failed\n", __func__);
			assert(0);
		}
	}
#endif /* end of PL_BOOTTAGS_BASE */
...
void mrdump_init(void *fdt) __attribute__((weak));
void mrdump_init(void *fdt)
{
}

int mrdump_check_enable(void) __attribute__((weak));
