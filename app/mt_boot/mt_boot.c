		   ...
		   void *ramdisk, unsigned ramdisk_sz)
{
	...
	#ifdef PRIZE_ADD_CMDLINE_LPDDRINFO
	char *hardinfo_pbuf;
	#endif
	/* end */
...
/* prize addded by wangmengdong for hardware info, ufs life, 20210206,start */
#if defined(MTK_UFS_SUPPORT)
	int *ufs_life = ufs_get_health_status();
	char life_time[10] = "000";
	char pre_eol_info[10] = "000";
	void *lk_drv_fdt = g_fdt;//prize huarui
#endif
/* prize addded by wangmengdong for hardware info, ufs life, 20210206,end */
...
/* prize addded by wangmengdong for hardware info, ufs life, 20210206,start */
#if defined(MTK_UFS_SUPPORT)
	snprintf(life_time,10,"0x%02x 0x%02x",ufs_life[1],ufs_life[2]);
	snprintf(pre_eol_info,10,"0x%02x",ufs_life[0]);
	pal_log_err("ufs life:%s  pre_eol_info:%s\n",life_time,pre_eol_info);
	ret = fdt_setprop(fdt, offset, "life_time", life_time, 10);
	if(ret)
		pal_log_err("fail to fdt_setprop life_time\n");
	
	ret = fdt_setprop(fdt, offset, "pre_eol_info", pre_eol_info, 10);
	if(ret)
		pal_log_err("fail to fdt_setprop pre_eol_info\n");
#endif
/* prize addded by wangmengdong for hardware info, ufs life, 20210206,start */
#else
	pal_log_err("PASS memory DTS node\n");
...
/* end */
	...
	//prize-modify-pengzhipeng-20200306-start
			snprintf(tmpbuf, TMPBUF_SIZE, "%s%d", "boot_reason=", boot_reason);
			cmdline_append(tmpbuf);
	//prize-modify-pengzhipeng-20200306-end
...
static inline int read_product_usbid(char *serialno)
{
	...
	/*PRIZE-uique serial-tangan-20170920-start*/
		...
		memset( cur_serialp, 0, sizeof(cur_serialp));
        int temp_i = 0;
...
//prize-Automatic test of fixed USB port-pengzhipeng-20210201-start
#ifndef PRIZE_FACTORY_UNIQUE_SERIAL
#ifdef CONFIG_MTK_USB_UNIQUE_SERIAL
...
#endif
//prize-Automatic test of fixed USB port-pengzhipeng-20210201-end
