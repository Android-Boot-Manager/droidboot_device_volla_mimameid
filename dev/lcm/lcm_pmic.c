...
void *lk_drv_fdt = get_lk_overlayed_dtb();//prize huarui
	...
	if (offset < 0) {
		...
		data = fdt_getprop(lk_drv_fdt, offset, "gpio_lcd_fd_pin", &len);
		if (len > 0 && data){
			gpio_lcd_fd_pin = fdt32_to_cpu(*(unsigned int *)data);
			dprintf(INFO, "[lcm_pmic] gpio_lcd_fd_pin(%d)\n",gpio_lcd_fd_pin);
		}
...
#endif
	display_bias_vpos_set(mv);
	display_bias_vneg_set(mv);
	return ret;
}
...
int display_bias_vpos_set(int mv)
{
	...
	ret = lcm_i2c_write_byte(0x00, mv);
	if (ret != I2C_OK){
...
int display_bias_vneg_set(int mv)
{
	...
	ret = lcm_i2c_write_byte(0x01, mv);
	if (ret != I2C_OK){
		dprintf(CRITICAL, "[lcm_pmic]cmd=%0x--i2c write error %d\n", 0x01,ret);
	}
...
int display_fd_enable(int enable)	//AMOLED
{
	int ret = 0;
#if defined(GPIO_LCD_LDO28_PIN)
	if (enable){
		mt_set_gpio_mode(GPIO_LCD_FD_PIN, GPIO_MODE_00);//define in dws
		mt_set_gpio_dir(GPIO_LCD_FD_PIN, GPIO_DIR_OUT);
		mt_set_gpio_out(GPIO_LCD_FD_PIN, GPIO_OUT_ONE);
	}else{
		mt_set_gpio_mode(GPIO_LCD_FD_PIN, GPIO_MODE_00);//define in dws
		mt_set_gpio_dir(GPIO_LCD_FD_PIN, GPIO_DIR_OUT);
		mt_set_gpio_out(GPIO_LCD_FD_PIN, GPIO_OUT_ZERO);
	}
#elif defined(USE_DTB_NO_DWS)
	if (!is_init_lcm_ldo_gpio){
		init_lcm_ldo_gpio_form_dtb();
	}
	if (gpio_lcd_fd_pin >= 0){
		if (enable){
			mt_set_gpio_mode(0x80000000 | gpio_lcd_fd_pin, GPIO_MODE_00);//define in dws
			mt_set_gpio_dir(0x80000000 | gpio_lcd_fd_pin, GPIO_DIR_OUT);
			mt_set_gpio_out(0x80000000 | gpio_lcd_fd_pin, GPIO_OUT_ONE);
		}else{
			mt_set_gpio_mode(0x80000000 | gpio_lcd_fd_pin, GPIO_MODE_00);//define in dws
			mt_set_gpio_dir(0x80000000 | gpio_lcd_fd_pin, GPIO_DIR_OUT);
			mt_set_gpio_out(0x80000000 | gpio_lcd_fd_pin, GPIO_OUT_ZERO);
		}
	}
#endif
	return ret;
