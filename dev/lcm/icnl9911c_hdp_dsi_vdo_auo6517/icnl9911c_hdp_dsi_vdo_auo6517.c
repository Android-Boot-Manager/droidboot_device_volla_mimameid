...
static void lcm_init(void)
{
	...
	lcm_set_bias(1);  // 5.8v
	MDELAY(20);//100
	SET_RESET_PIN(1);  //lcd reset 1
	MDELAY(10);//100
	SET_RESET_PIN(0);  //lcd reset 0
	MDELAY(10);//100
	SET_RESET_PIN(1);  //lcd reset 1
...
static unsigned int lcm_compare_id(void)
{
	...
	unsigned int id1 = 0;
	unsigned int i = 0;
	unsigned char buffer[2],buffer1[2];
...
//	int i=0;
	...
	for(i=0;i<10;i++){
		...
		// lcm_set_bias(0);
		MDELAY(50);
		//if ((lcm_vol >= MIN_VOLTAGE && lcm_vol <= MAX_VOLTAGE) && (id == 0x9911))
		if (id == 0x9911)
		{
			printf("%s,nl9911_id=0x%x,id0=0x%x,id1=0x%x\n", __func__,id,buffer[0],buffer[1]);
			return 1;
		}
	}
    return 0;
...
LCM_DRIVER icnl9911c_hdp_dsi_vdo_auo6517_lcm_drv = {
