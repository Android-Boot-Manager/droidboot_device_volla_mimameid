...
void ccci_update_lk_arg_info_to_dt(char *buf, void *fdt, int offset)
{
	...
	if (update_lk_arg_info_to_dt) {
		...
		if (fdt_setprop(fdt, offset, "ccci,modem_info_v2", buf, ptr - buf)) {
			...
			dprintf(CRITICAL, "[ccci] create modem chosen arguments info DT OK\n");
#if 1

		nodeoffset = fdt_path_offset(fdt, "/mddriver");
		if (nodeoffset < 0) {
			dprintf(CRITICAL, "/mddriver not found, try /soc/mddriver\n");
			nodeoffset = fdt_path_offset(fdt, "/soc/mddriver");
			if (nodeoffset < 0) {
				dprintf(CRITICAL, "/soc/mddriver not found\n");
				return;
			}
		}
#endif

		if (fdt_setprop(fdt, nodeoffset, "ccci,modem_info_v2", buf, ptr - buf)) {
			assert(0);
			return;
		} else
			dprintf(CRITICAL, "[ccci] create modem mddriver arguments info DT OK\n");
	} else
