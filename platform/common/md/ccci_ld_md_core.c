...
static int find_hdr_in_partiton(char *partition_name, char *hdr_name, union mkimg_hdr *p_hdr, unsigned int p_hdr_size)
{
	...
	while(1) {
		...
		if (p_hdr->info.magic != MKIMG_MAGIC) {
			ALWAYS_LOG("invalid magic(%x):(%x)ref @%s_%s\n", p_hdr->info.magic,
					MKIMG_MAGIC, partition_name, hdr_name);
...
static int load_main_image(download_info_t *main_img, unsigned char **base_addr, int *resv_mem_size)
{
	...
	md_mem_base = ccci_request_named_mem("md_mem_usage", main_img->max_size);
	if (md_mem_base == NULL) {
...
static int ld_img_at_list(download_info_t img_list[], modem_info_t *info)
{
	int load_size;
	...
	while (curr->img_type != 0) {
		...
		if ((load_size >= 0) && (load_size > curr->img_size)) {
			...
			if (curr->ext_flag & LK_IMG_OPTINAL) {
				ALWAYS_LOG("skip sub image: %s fail with ret info: %s\n",
...
static download_info_t md1_download_list_v30000[] = {/* for 97 */
	/* img type | partition | image name | max size  | img size | ext_flag         | mem addr | ass func p */
	{main_img,   "md1img",   "md1rom",    0x20000000,   0,         0,                NULL,      ass_func_for_v6_normal_img},
	{dsp_img,    "md1dsp",   "md1dsp",    0x4000000,    0,         0,                NULL,      ass_func_for_dsp_normal_img},
	{armv7_img,  "md1arm7",  "md1arm7",   0x200000,    0,         0,                NULL,      NULL},
	{drdi_img,   "md1img",   "md1drdi",   0x4000000,     0,         0,                NULL,      NULL},
	{0,          NULL,       NULL,        0,           0,         0,                NULL,      NULL},
};
...
static download_info_t md1_download_list_v20001[] = {/* for 92 */
	/* img type | partition | image name | max size  | img size | ext_flag         | mem addr */
	{main_img,   "md1img",    "md1rom",    0x20000000,  0,         DUMMY_AP_FLAG_MAIN,    NULL,     ass_func_for_v6_md_only_img},
...
__WEAK size_t mt_secure_call_all(size_t function_id,
	size_t arg0, size_t arg1, size_t arg2,
	size_t arg3, size_t *r1, size_t *r2, size_t *r3)
{
	ALWAYS_LOG("[%s] not support this func.\n", __func__);

	return (size_t)-1;
}

static void set_hw_locked_in_atf(void)
{
	size_t ret;
	ALWAYS_LOG("[%s]\n", __func__);
	ret = mt_secure_call(MTK_SIP_BL_CCCI_CONTROL_AARCH32,
			MD_HW_REMAP_LOCKED,	0, 0, 0);
	if (ret)
		ALWAYS_LOG("[%s] error: MD_HW_REMAP_LOCKED fail: 0x%zX\n", __func__, ret);
	else
		ALWAYS_LOG("[%s] call MD_HW_REMAP_LOCKED succ.\n", __func__);
}
...
void load_modem_image(void)
{
	...
	/* 3. tel info init */
	md_bitmap = ccci_ld_md_tel_init();
	if (!md_bitmap) {
		err_code = -LD_ERR_PLAT_INVALID_MD_ID;
		goto _err_exit;
	}
	...
	if (md_bitmap & MD1) {
		...
		switch (ld_img_ver) {
			case 0x10001:
				ret = ld_img_at_list(md1_download_list_v10001, &info);
				break;
			case 0x10000:
				ret = ld_img_at_list(md1_download_list, &info);
				break;
			case 0x20000:
				ret = ld_img_at_list(md1_download_list_v20000, &info);
				break;
			case 0x20001:
				ret = ld_img_at_list(md1_download_list_v20001, &info);
				break;
			case 0x30000:
				ret = ld_img_at_list(md1_download_list_v30000, &info);
				break;
			case 0x40000:
				ret = ld_img_at_list(md1_download_list_v40000, &info);
				break;
			default:
				ret = ld_img_at_list(md1_download_list, &info);
				break;
...
_load_md3:
	if (md_bitmap & MD3) {
		...
		if (ccci_hal_get_ld_md_plat_setting("ld_version") == 0x10001 ||
			...
			ret = ld_img_at_list(md3_download_list_v10001, &info);
		else
			ret = ld_img_at_list(md3_download_list, &info);
		if (ret < 0) {
...
_load_end:
	...
	smem_addr = ccci_request_named_mem("ap_md_nc_smem", smem_size);
	if (smem_addr == NULL) {
...
void ccci_update_md_version(int md_id, unsigned char ver[])
{
	...
	if (md_id == MD_SYS1) {
		md1_version_rdy = 1;
		ret = snprintf(md1_version, 65, "%s", ver);
	} else if (md_id == MD_SYS3) {
		md3_version_rdy = 1;
		ret = snprintf(md3_version, 65, "%s", ver);
	}
	if (ret < 0 || ret >= 65)
		ALWAYS_LOG("%s-%d:snprintf fail,ret=%d\n",
