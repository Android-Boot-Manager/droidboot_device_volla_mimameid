...
void *ccci_request_named_mem(char *name, unsigned int mem_size)
{
	void *resv_addr;
	...
	ALWAYS_LOG("name[%s] limit: 0x%llX, request size: 0x%08x, get start address: %p\n", name, limit, mem_size, resv_addr);
	return resv_addr;
}
	...
	unsigned long *align)
{
	...
	if (strcmp(name, "ccci_tag_mem") == 0) {
		...
		*align = TAG_MEM_ALIGNMENT;
		return 0;
	} else if ((strcmp(name, "md_mem_usage") == 0) ||(strcmp(name, "md1rom") == 0) ||
	(strcmp(name, "md3rom") == 0)) {
	...
	} else if ((strcmp(name, "ap_md_c_smem") == 0) || (strcmp(name, "ap_md_nc_smem") == 0)) {
...
unsigned long long resv_sib_named_memory(char name[], unsigned int size)
{
	unsigned long long limit;
	unsigned long align;
	unsigned long long resv_addr;
	...
	resv_addr = (unsigned long long)mblock_reserve_ext(&g_boot_arg->mblock_info, size, align, limit, 1, name);
	ALWAYS_LOG("name[%s] limit: 0x%llX, request size: 0x%08x, get start address: 0x%llx\n", name, limit, size, resv_addr);
	return resv_addr;
}
...
int ccci_load_raw_data(char *part_name, unsigned char *mem_addr, unsigned int offset, int size)
{
	...
	arch_sync_cache_range((addr_t)mem_addr, (size_t)size);
	ALWAYS_LOG("ccci_load_raw_data: addr[%p],size[0x%08x] @%s\n", mem_addr, size, part_name);
