...
static int is_gpt_header_valid(gpt_header * header, u64 header_lba);
static int is_gpt_entries_valid(gpt_header * header, gpt_entry * entries);
...
static int parse_gpt_header(u32 part_id, u64 header_lba, u8 *header_buf, u8 *entries_buf)
{
	int i;
	int err;
	u64 entries_read_size, num_block_entry;
	part_dev_t *dev = mt_part_get_device();
	...
	if (is_gpt_header_valid(header, header_lba) != 0)
		return 1;
	...
	if (is_gpt_entries_valid(header, entries) != 0)
		return 1;
...
static int is_gpt_header_valid(gpt_header * header, u64 header_lba)
{
	...
	if (header->signature != GPT_HEADER_SIGNATURE) {
		efi_err("%s check header, err(signature 0x%llx!=0x%llx)\n",
				TAG, header->signature, GPT_HEADER_SIGNATURE);
		return -1;
	}
	// header crc32 check
	...
	if (header->num_partition_entries > PART_MAX_COUNT) {
		efi_err("%scheck header, num_partition_entries(%d) exceeds\n",
				TAG, header->num_partition_entries);
		return -1;
	}
...
int is_gpt_valid(const u8 * gpt_raw, u64 header_lba)
{
	gpt_header *header;
	gpt_entry *entries;
	part_dev_t *dev = mt_part_get_device();

	header = (gpt_header *)gpt_raw;
	entries = (gpt_entry *)(gpt_raw + dev->blkdev->blksz);
	if (is_gpt_header_valid(header, header_lba) != 0 ||
		is_gpt_entries_valid(header, entries) != 0)
		return -1;
	return 0;
}
