...
int partition_get_index(const char * name)
{
	...
	if (part_suffix != NULL) {
		if (!(part_suffix[0] == last_char[0] && part_suffix[1] == last_char[1])) {
			len_ck = snprintf(alt_name, PART_META_INFO_NAMELEN + BOOTCTRL_SUFFIX_MAXLEN, "%s%s", name, part_suffix);
			if (len_ck < 0) {
				pal_log_err(PART_COMMON_TAG"%s snprintf string %d into %d\n",
					__func__, PART_META_INFO_NAMELEN + BOOTCTRL_SUFFIX_MAXLEN, len_ck);
				return -1;
			}
			check_alt_name = 1;
		}
...
static void partition_commandline_bootdevice(void)
{
...
#endif
#ifdef UFS_HCI_BASE
	ret = snprintf(bootdev_buf, BOOT_DEV_BUF_SIZE, ",soc/%08x.ufshci", UFS_HCI_BASE);
	if (ret < 0) {
		pal_log_err(PART_COMMON_TAG"append UFS_HCI_BASE(0x%08x) to command line fail\n", UFS_HCI_BASE);
		return;
	}
	boot_device = strncat(boot_device, bootdev_buf, remain);
	if (boot_device == NULL) {
	...
	remain -= ret;
		ret = snprintf(bootdev_buf, BOOT_DEV_BUF_SIZE, ",%08x.ufshci", UFS_HCI_BASE);
	if (ret < 0) {
		pal_log_err(PART_COMMON_TAG"append UFS_HCI_BASE(0x%08x) to command line fail\n", UFS_HCI_BASE);
		return;
	}
...
int part_init(part_dev_t *dev)
{
	...
	part_pgpt->info = info;
	len_ck = snprintf((char*)part_pgpt->info->name, strlen("pgpt")+1, "%s", "pgpt");
	if (len_ck < 0) {
		pal_log_err(PART_COMMON_TAG"%s snprintf string %d into %d\n",
			__func__, strlen("pgpt")+1, len_ck);
		return -1;
	}
	...
	part_sgpt->info = info;
	len_ck = snprintf((char*)part_sgpt->info->name, strlen("sgpt")+1, "%s", "sgpt");
	if (len_ck < 0) {
		pal_log_err(PART_COMMON_TAG"%s snprintf string %d into %d\n",
			__func__, strlen("sgpt")+1, len_ck);
		return -1;
	}
	...
	if (g_partition_all[0].info) {
		len_ck = snprintf((char*)g_partition_all[0].info->name, strlen("preloader")+1, "%s", "preloader");
		if (len_ck < 0) {
			pal_log_err(PART_COMMON_TAG"%s snprintf string %d into %d\n",
				__func__, strlen("preloader")+1, len_ck);
			return -1;
		}
	}
#ifdef MTK_AB_OTA_UPDATER
	...
	if (g_partition_all[1].info) {
		len_ck = snprintf((char*)g_partition_all[1].info->name, strlen("preloader_a")+1, "%s", "preloader_a");
		if (len_ck < 0) {
			pal_log_err(PART_COMMON_TAG"%s snprintf string %d into %d\n",
				__func__, strlen("preloader_a")+1, len_ck);
			return -1;
		}
	}
	...
	if (g_partition_all[2].info) {
		len_ck = snprintf((char*)g_partition_all[2].info->name, strlen("preloader_b")+1, "%s", "preloader_b");
		if (len_ck < 0) {
			pal_log_err(PART_COMMON_TAG"%s snprintf string %d into %d\n",
				__func__, strlen("preloader_b")+1, len_ck);
			return -1;
		}
	}
#endif
...
int get_gpt_entry_index(const char * part_name)
{
	int i;

	for (i=0; i<PART_MAX_COUNT; i++)
		if (strncmp((const char *)g_partition_all[i].info->name, "pgpt", strlen("pgpt")) != 0 &&
			strncmp((const char *)g_partition_all[i].info->name, "preloader", strlen("preloader")) != 0)
				return partition_get_index_by_name(part_name) - i;

	pal_log_err(PART_COMMON_TAG"fail to get index in %s\n", __func__);
	return -1;
}
