...
static const char *boot_partitions[] = {"boot", NULL, NULL};
...
static int32_t build_up_boot_partitions(const char **req_parts, uint32_t parts_sz)
{
	uint32_t i;
	int32_t ret = 0;

	/* add vendor_boot in request partitions */
	if (partition_exists("vendor_boot") == PART_OK) {
		for (i = 0 ; i < parts_sz ; i++) {
			if (req_parts[i] == NULL) {
				req_parts[i] = "vendor_boot";
				break;
			}
		}
		if (i >= parts_sz) {
			pal_log_err("Not enough space to add vendor_boot to request partitions\n");
			ret = -ENOMEM;
		}
	}
	return ret;
				     ...
				     AvbSlotVerifyData *slot_data)
{
	...
	for (i = 0; i < slot_data->num_loaded_partitions; i++) {
			boot_load_addr = bootimg_alloc_work_buf(slot_data->loaded_partitions[i].partition_name,
				slot_data->loaded_partitions[i].data_size);
			if (!boot_load_addr)
				panic("fail to allocate boot img workbuf\n");
	...
	suffix = get_suffix();
	if (suffix == NULL) {
		pal_log_err("[avb] Fail to get suffix\n");
	...
	if (ret)
		goto end;
#ifdef MTK_AB_OTA_UPDATER
	if (snprintf(vboot_cmdline_ab, TMPBUF_SIZE,
...
#endif
	set_bootimg_loaded((uint32_t)boot_load_addr);
...
int load_vfy_boot(uint32_t bootimg_type, uint32_t addr)
{
	...
	ret = vboot_state_result(bootimg_type);
	if (ret != STATUS_OK)
		return ret;
