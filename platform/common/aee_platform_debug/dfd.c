...
static void dfd_decode_return_stack(const u64 *dfd_raw_data)
{
	...
	if (!return_stack) {
		dprintf(CRITICAL, "[dfd] %s, malloc failed\n", __func__);
		return;
	}
	...
	for (i = 0; i < cfg_pc_latch.nr_max_big_core; ++i) {
		if (cfg_dfd.sw_version == DFD_SW_V1) {
			...
			if (lastpc[cfg_big_core[i].cpuid].power_state != DFD_CORE_PWR_ON
			        && lastpc[cfg_big_core[i].cpuid].power_state != DFD_CORE_PWR_RETENTION)
				continue;
#ifdef DFD_USE_LAST_BRANCH
			lastpc[cfg_big_core[i].cpuid].pc = dfd_decode(dfd_raw_data, &(big_core[i].last_branch), true);
#else
			lastpc[cfg_big_core[i].cpuid].pc = dfd_decode(dfd_raw_data, &(big_core[i].i0_pc), true);
#endif
		}
...
void __attribute__((weak)) plat_dump_dfd_status(void)
{
	dprintf(CRITICAL,"[DFD] Please implement plat_dump_dfd_status()\n");
}
...
static unsigned int dfd_internal_dump_check_triggered_or_not(void)
{
	...
	if (cfg_dfd.version >= DFD_V3_0) {
		if (!get_dbg_info_base) {
			dprintf(CRITICAL, "[dfd] get_dbg_info_base is NULL\n");
			return 0;
		}
		plat = (DEF_PLAT_SRAM_FLAG *)get_dbg_info_base(PLAT_SRAM_FLAG_KEY);
		if (!plat) {
			dprintf(CRITICAL, "[dfd] get_dbg_info_base(PLAT_SRAM_FLAG_KEY) is NULL\n");
			return 0;
		}
		...
		if ((plat->plat_sram_flag1 & 0x1) && (get_efuse_dfd_disabled() == 0x0)) {
			if (g_boot_arg != NULL &&
				...
				dprintf(CRITICAL, "[dfd] ddr_reserve_enable = 0x%x, ddr_reserve_success = 0x%x\n",
						g_boot_arg->ddr_reserve_enable, g_boot_arg->ddr_reserve_success);
			}
		}
		else {
			dprintf(CRITICAL, "[dfd] plat_sram_flag1 = 0x%x\n", plat->plat_sram_flag1);
			dprintf(CRITICAL, "[dfd] get_efuse_dfd_disabled() = 0x%x\n", get_efuse_dfd_disabled());
			dfd_internal_dump_triggered = 0;
		}
	...
	plat_dump_dfd_status();

	dprintf(CRITICAL, "[dfd] dfd_internal_dump_triggered = 0x%x\n", dfd_internal_dump_triggered);
	...
	*data = NULL;
	...
	if (dfd_internal_dump_check_triggered_or_not()) {
		...
		if (dfd_dump_type == DFD_DUMP_TO_DRAM) {
			/*
			 * use DRAM: need mapping to scratch memory as temp buffer before accessing
		...
		if (dfd_op.check_dfd_valid) {
			if (dfd_op.check_dfd_valid((u32 *)*data)) {
				...
				*len = cfg_dfd.buffer_length;
				...
				/* restore the map of SCRATCH_ADDR */
				if (dfd_dump_type == DFD_DUMP_TO_DRAM) {
					if (cfg_dfd.large_buffer_length)
						arch_mmu_map(vaddr, vaddr,
						MMU_MEMORY_TYPE_NORMAL_WRITE_BACK | MMU_MEMORY_AP_P_RW_U_NA,
						cfg_dfd.large_buffer_length);
					else
						arch_mmu_map(vaddr, vaddr,
						MMU_MEMORY_TYPE_NORMAL_WRITE_BACK | MMU_MEMORY_AP_P_RW_U_NA,
						cfg_dfd.buffer_length);
					*data = paddr;
				}
				dprintf(CRITICAL, "[dfd] pa = 0x%llx, va = 0x%lx, length = 0x%x\n",
					paddr, vaddr, *len);
				dprintf(CRITICAL, "[dfd] check_dfd_valid fail\n");
				goto dfd_end;
			}
		} else {
			dfd_internal_dump_decode_lastpc((u64 *)*data);
