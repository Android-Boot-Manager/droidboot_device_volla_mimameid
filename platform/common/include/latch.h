...
struct lastbus_ops_v2 {
	int (*init)(const struct lastbus_monitor *m);
	bool (*is_timeout)(const struct lastbus_monitor *m);
	int (*dump)(const struct lastbus_monitor *m, char *buf, int *wp);
};
...
struct plt_cfg_bus_latch {
	...
	struct plt_cfg_bus_latch_v2 v2;
