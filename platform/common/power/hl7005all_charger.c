...
{	
	...
	offset = fdt_node_offset_by_compatible(get_lk_overlayed_dtb(),0,"halo,hl7005");
	if (offset < 0){
		dprintf(CRITICAL, "[hl7005] Failed to find halo,hl7005 in dtb\n");
		return -1;
	}else{
		data = fdt_getprop(get_lk_overlayed_dtb(), offset, "gpio_swcharger_en_pin", &len);
		if (len > 0 && data){
			info->gpio_sw_en = fdt32_to_cpu(*(unsigned int *)data);
			dprintf(INFO, "[hl7005] gpio_swcharger_en_pin(%d)\n",info->gpio_sw_en);
		}else{
			dprintf(INFO, "[hl7005] get gpio_swcharger_en_pin fail, default(%d)\n",info->gpio_sw_en);
		}
		...
		if (len > 0 && data){
			...
			dprintf(INFO, "[hl7005] get i2c id fail, default(%d)\n",info->i2c.id);
		}
	}
	return 0;
}
