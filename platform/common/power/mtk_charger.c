...
//prize-add-pengzhipeng-20191104-start
	...
	if (ret < 0)
	{
		...
		if (batvolt <= 0){
			...
			//batvolt = get_bat_volt(1); //if failed and get pmic's Vbat
			cw2015_init_succ = 0;
		}
	}

	dprintf(CRITICAL, "cw2015 ===batvolt= %d, batcap=%d\n", batvolt,batcap);
#elif defined(MTK_CW2017_SUPPORT)
	int batcap = 0;
	int batvolt = 0;
	int ret = 0;
	int cw2017_init_succ = 1;
	ret = cw2017_init(); //hjian
	if (ret < 0)
	{
		mdelay(20);
		ret = cw2017_init();
		if (ret < 0)
		{
			dprintf(CRITICAL, "%s, cw2017 init failed(%d)\n", __FUNCTION__,ret);
			cw2017_init_succ = 0;
		}
	}else{
		batcap = cw2017_get_capacity();
		if (batcap < 0 || batcap > 100){
			dprintf(CRITICAL, "cw2017 get cap failed.(%d)\n",batcap);
			cw2017_init_succ = 0;
		}
		batvolt = cw2017_get_voltage();
		if (batvolt <= 0){
			dprintf(CRITICAL, "cw2017 get vol failed.(%d)\n",batvolt);
			cw2017_init_succ = 0;
		}
	}
	dprintf(CRITICAL, "cw2017 ===batvolt= %d, batcap=%d\n", batvolt,batcap);
#endif
//prize-add-pengzhipeng-20191104-end
...
#if defined(MTK_CW2015_SUPPORT)
	if (cw2015_init_succ != 1){
		dprintf(CRITICAL, "CW2015 init fail, set fake batcap=1\n");
		batcap = 1;
	}
	if ((val < BATTERY_LOWVOL_THRESOLD) || (batcap < 1)) {
#elif defined(MTK_CW2017_SUPPORT)
	if (cw2017_init_succ != 1){
		dprintf(CRITICAL, "CW2017 init fail, set fake batcap=1\n");
		batcap = 1;
	}
	if ((val < BATTERY_LOWVOL_THRESOLD) || (batcap < 1)) {
#else
...
//prize modified by huarui, eta6937 support, 20190305-start
		if (mtk_charger_check_alias_name(primary_mchr,"hl7005")){
