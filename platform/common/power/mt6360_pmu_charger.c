...
static int mt_charger_enable_discharge(struct mtk_charger_info *mchr_info, bool en)
{
	struct mt6360_pmu_chg_info *info =
		(struct mt6360_pmu_chg_info *)mchr_info;
	int ret = 0;
	...
	ret = mt6360_i2c_update_bits(info, MT6360_PMU_CHG_HIDDEN_CTRL2,
...
static int mt_charger_sw_reset(struct mtk_charger_info *mchr_info)
{
	struct mt6360_pmu_chg_info *info =
		(struct mt6360_pmu_chg_info *)mchr_info;
	int ret = 0;
	u32 i = 0;
	u8 cmds[3] = {MT6360_PMU_RST_PAS_CODE1, MT6360_PMU_RST_PAS_CODE2,
		      MT6360_PMU_RST1};
	u8 data[3] = {0xA9, 0x96, 0x7F};

	for (i = 0; i < ARRAY_SIZE(cmds); i++) {
		ret = mt6360_i2c_write_byte(info, cmds[i], data[i]);
		if (ret < 0) {
			dprintf(CRITICAL, "%s: fail(%d)\n", __func__, ret);
			return ret;
		}
	...
	mdelay(1);
	for (i = 0; i < ARRAY_SIZE(cmds); i++) {
		ret = mt6360_i2c_write_byte(info, cmds[i], 0x00);
		if (ret < 0) {
			dprintf(CRITICAL, "%s: fail(%d)\n", __func__, ret);
			return ret;
		}
	}
	return mt6360_clr_bit(info, MT6360_PMU_CORE_CTRL1, MT6360_MASK_MREN);
}
...
static struct mtk_charger_ops mt6360_mchr_ops = {
	...
	.enable_discharge = mt_charger_enable_discharge,
	.sw_reset = mt_charger_sw_reset,
};
