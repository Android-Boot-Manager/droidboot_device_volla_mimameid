...
/* prize addded by wangmengdong for hardware info, ufs life, 20210206,start */
	...
	if (err) {
		...
		card_data->life_time_est_a = p[HEALTH_DESC_B_DEVICE_LIFE_TIME_EST_A];
		card_data->life_time_est_b = p[HEALTH_DESC_B_DEVICE_LIFE_TIME_EST_B];
		UFS_DBG_LOGE("[UFS] QUERY_DESC_IDN_HEALTH\n");
		ufs_aio_util_dump_table_hex(p, ufs_query_desc_max_size[QUERY_DESC_IDN_HEALTH]);
	}
	for(int i=0;i<3;i++)
	{
		health_buf[i] = p[i+2];
		UFS_DBG_LOGE("[UFS] health_buf[%d]=%x\n",i,health_buf[i]);
	}
#endif
/* prize addded by wangmengdong for hardware info, ufs life, 20210206,end */
...
/* prize addded by wangmengdong for hardware info, ufs life, 20210206,start */
#if defined(MTK_UFS_SUPPORT)
int *ufs_get_health_status(void)
{
	int i;
	for(i=0;i<3;i++)
	UFS_DBG_LOGE("[UFS] health_buf[%d]=%x\n",i,health_buf[i]);
