...
unsigned char g_ufs_4k_buf[UFS_BLOCK_SIZE] __attribute__((aligned(UFS_PLATFORM_CACHE_LINE_SIZE)));
#endif
...
int ufs_aio_dma_erase(struct ufs_hba * hba, u32 lun, u32 blk_start, u32 blk_cnt)
{
	...
	for (i = 0; i < wr_blk_cnt; i++)
		ufs_aio_block_write(0, lun, blk_start + i, 1, (unsigned long *)g_ufs_4k_buf);
#endif
